...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const isTr = lang === 'tr';
  const [testEmail, setTestEmail] = useState('qa.testuser@flowshop.mock');
  const [testPassword, setTestPassword] = useState('FlowShopQA!2026Secure');
  const [showPw, setShowPw] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [savedSuccess, setSavedSuccess] = useState(false);
  const [activeProjectId, setActiveProjectId] = useState('proj-flowshop-tr');
        ...
        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-3">
          ...
          <div className="space-y-2 text-xs text-slate-600 dark:text-slate-300 font-mono">
            ...
            <div className="flex justify-between p-2.5 bg-slate-50 dark:bg-slate-900/60 rounded-xl">
              ...
              <strong className="text-indigo-600 dark:text-indigo-400">https://flowshop-tr.mock/</strong>
            </div>
            <div className="flex justify-between p-2.5 bg-slate-50 dark:bg-slate-900/60 rounded-xl">
              <span>Telemetry:</span>
