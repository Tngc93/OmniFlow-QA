...
async function run() {
  ...
  try {
    ...
    // Verify healed cards
    const isMonsterHealedVisible = await page.locator('text=Monster TR Sepete Ekle Butonu').isVisible();
    const isTulparHealedVisible = await page.locator('text=Tulpar DE QWERTZ Klavye Konfigüratörü').isVisible();
    console.log('Monster TR Healed Selector Visible:', isMonsterHealedVisible);
    console.log('Tulpar DE Healed Selector Visible:', isTulparHealedVisible);
    ...
    fs.writeFileSync(path.join(ARTIFACT_DIR, 'code_and_healing_audit.json'), JSON.stringify({
      ...
      monsterHealedVisible: isMonsterHealedVisible,
      tulparHealedVisible: isTulparHealedVisible,
      consoleErrors,
      networkWarnings
    }, null, 2));
